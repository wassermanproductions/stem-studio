/**
 * Parser for the Python worker's line-delimited JSON stdout. A stream-safe
 * LineParser buffers partial chunks; parseWorkerLine validates a single line
 * into a typed WorkerEvent (or null for blank/non-JSON noise).
 */

import type { WorkerEvent, WorkerStage } from './types'
...
export function parseWorkerLine(line: string): WorkerEvent | null {
  ...
  switch (rec.event) {
    ...
    case 'error': {
      const message = typeof rec.message === 'string' ? rec.message : 'Unknown worker error'
...
/**
 * Buffers raw stdout chunks and yields complete-line WorkerEvents. Retains
 * any trailing partial line until the next chunk completes it.
 */
