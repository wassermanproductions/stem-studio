...
if (!['windows-x64', 'macos-arm64', 'linux-arm64'].includes(target)) {
  throw new Error(
    'A release target is required: --target windows-x64|macos-arm64|linux-arm64'
  )
}
const platform = target === 'windows-x64'
  ? 'windows'
  : target === 'macos-arm64'
    ? 'mac'
    : 'linux'
...
const macMedia = platform === 'mac'
  ? JSON.parse(
      await readFile(resolve(root, 'build/runtime/macos-arm64/media-provenance.json'), 'utf8')
    )
  : null

const platformRuntimeComponents = platform === 'windows'
  ? [
  ...
  : platform === 'mac'
    ? ['ffmpeg', 'ffprobe'].map((name) => ({
        ...
        license: 'GPL-3.0-or-later',
        hashes: [{ alg: 'SHA-256', content: macMedia.files[name] }],
...
console.log(
  ...
  `for ${target} (${mcpComponents.length} bundled MCP components).`
)
